...
CREATE TABLE IF NOT EXISTS openvaia.conversations (
...
CREATE INDEX IF NOT EXISTS idx_conversations_convid_created
    ON openvaia.conversations (conversation_id, created_at);

CREATE INDEX IF NOT EXISTS idx_conversations_agent_created
    ON openvaia.conversations (agent_name, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_conversations_channel_created
    ON openvaia.conversations (channel_type, created_at DESC);

-- Memory store (hybrid FTS + pgvector embedding search)
CREATE TABLE IF NOT EXISTS openvaia.memories (
    id          BIGSERIAL PRIMARY KEY,
    agent_name  VARCHAR(64) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_memories_agent_tier
    ON openvaia.memories (agent_name, tier);

CREATE INDEX IF NOT EXISTS idx_memories_tags
    ON openvaia.memories USING GIN (tags);

CREATE INDEX IF NOT EXISTS idx_memories_fts
    ON openvaia.memories USING GIN (to_tsvector('english', content));

CREATE INDEX IF NOT EXISTS idx_memories_embedding
    ON openvaia.memories USING hnsw (embedding vector_cosine_ops);

-- Research reports
CREATE TABLE IF NOT EXISTS openvaia.research_reports (
...
CREATE INDEX IF NOT EXISTS idx_research_agent_created
    ON openvaia.research_reports (agent_name, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_research_tags
    ON openvaia.research_reports USING GIN (tags);

CREATE INDEX IF NOT EXISTS idx_research_fts
    ON openvaia.research_reports USING GIN (
        to_tsvector('english', title || ' ' || summary)
    );

-- Seed platform config values
INSERT INTO openvaia.config (key, value, description) VALUES
...
DROP TABLE IF EXISTS openvaia.research_reports;
DROP TABLE IF EXISTS openvaia.memories;
DROP TABLE IF EXISTS openvaia.conversations;
DELETE FROM openvaia.config WHERE key IN (
